...
-- Custom (Jacob-typed) Latin vocab words added via SoundStudio's "+ Add your own word
-- or phrase" (RCA First Form Latin 6). Mirrors what's already saved to localStorage
-- client-side (src/lib/rca-custom-vocab.ts) so the words are durable across
-- browsers/devices AND visible to a script running locally on Jacob's own Mac
-- (scripts/gen-custom-vocab-audio.mjs) — edge-tts/whisper are CLI binaries, not
-- something Vercel's serverless functions can run, so real verified audio for a
-- custom word can only ever be generated by a local script reading this table; it
-- has no way to see localStorage. `id` matches the client-generated id from
-- useCustomVocab() so the two stay linked without a second lookup.
create table if not exists mt_custom_vocab (
  user_email text not null,
  id text not null,
  latin text not null,
  ...
  note text,
  audio_ready boolean not null default false, -- flips true once gen-custom-vocab-audio.mjs verifies a real mp3
  created_at timestamptz not null default now(),
  primary key (user_email, id)
);

create index if not exists idx_mt_custom_vocab_user on mt_custom_vocab(user_email);
